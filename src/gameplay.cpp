...
void gameplay::import_variable(){
    ...
    for (int i = 0; i < 6; i++){
        ...
        Rectangle tempDest = {1280.0f - (float)tempTex.width / 2 + 100, 50, 120, 200};
...
void gameplay::rendering(){
    origin = {0.0f, 0.0f};
    texture_rect = {x, y, (float)(office.width), (float)(office.height)};
...
void gameplay::mindbar_logic(){
    ...
    if(timer_mindbar >= 1.0f * coef_mindbar){
        status_mindbar--;
...
void gameplay::mindbar_rendering(){
    // string stmind = to_string(status_mindbar);
    // DrawText(stmind.c_str(), 0, 0, 100, WHITE);
    // string tmind = to_string(timer_mindbar);
    // DrawText(tmind.c_str(), 200, 0, 100, WHITE);
